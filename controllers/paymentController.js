...
export const initializePremiumPayment =
  async (req, res) => {
    try {
      if (
      ...
      if (
        ...
        return res.status(500).json({
          success: false,
          message:
            "Paystack Premium plan code is not configured.",
        });
      }
      ...
      const reference =
        `SONAR_PREMIUM_${Date.now()}_${crypto
          .randomBytes(5)
          .toString("hex")}`;
      ...
      return res.status(500).json({
        success: false,
        message:
          "Failed to initialize Premium payment.",
      });
    }
  };
  ...
  async (req, res) => {
    try {
      ...
      return res.status(500).json({
        success: false,
        message:
          "Failed to verify Premium payment.",
      });
    }
  };
